...
- **A filterable skill deck** — 52 skills from the résumé and LinkedIn in six groups (languages, frameworks, tools, ml & data, engineering, growth) behind a tab row, with the group counts summing to the total. The chips *slide* to their new positions using FLIP (measure First, mutate, measure Last, Invert the delta as a transform, then Play it off), so filtering reads as rearranging rather than repainting
