...
- **Work as an editor pane** — a tabbed file tree: a gutter of line numbers generated by a CSS counter (never hardcoded, so adding a role renumbers itself), `├──` connectors in a system monospace stack, and the current role marked with a live dot
...
`soft-light`. The aurora behind it is three blurred radial blobs in violet, purple
and deep violet (`#8b5cf6`, `#a855f7`, `#6d28d9`) at 9–26% alpha: enough for the
glass to have something to bend, faint enough to stay out of the way. Accent
`#8b5cf6` light / `#a78bfa` dark. The editor pane uses a system monospace stack, so
the connectors and gutter cost no extra font request.

## Structure

```
.
├── index.html   # Intro + widgets, about, work (editor pane), projects,
